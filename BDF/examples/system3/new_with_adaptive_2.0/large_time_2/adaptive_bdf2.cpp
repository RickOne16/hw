#include "adaptive_bdf2.hpp"
#include <boost/numeric/ublas/matrix.hpp>
#include <boost/numeric/ublas/vector.hpp>
#include <fstream>
typedef boost::numeric::ublas::vector< double > vector_type;
typedef boost::numeric::ublas::matrix< double > matrix_type;

struct stiff_system
{    
    void operator()( const vector_type &x , vector_type &dxdt , double /* t */ )
    {
        dxdt[ 0 ] = -0.04 * x[ 0 ] + 10000.0 * x[ 1 ] * x[ 2 ];
        dxdt[ 1 ] =  0.04 * x[ 0 ] - 10000.0 * x[ 1 ] * x[ 2 ] - 3 * 10000000.0 * x[ 1 ] * x[ 1 ];
        dxdt[ 2 ] =  3 * 10000000.0 * x[ 1 ] * x[ 1 ];
    }   
};

struct stiff_system_jacobi
{
     void operator()( const vector_type & x , matrix_type &J , const double & /* t */ )
    {
        J( 0 , 0 ) = -0.04;
        J( 0 , 1 ) = 10000.0 * x [2];
        J( 0 , 2 ) = 10000.0 * x[1];
        J( 1 , 0 ) = 0.04;
        J( 1 , 1 ) = -10000.0 * x[2] - 3 * 10000000.0 * 2 * x[1];
        J( 1 , 2 ) = -10000.0 * x[1];
        J( 2 , 0 ) = 0;
        J( 2 , 1 ) = 3 * 10000000.0 * 2 * x[1];
        J( 2 , 2 ) = 0;
    }
};
using namespace std;
int main()
{

    ofstream myfile;
    myfile.open( "adbdf2.txt");
     vector_type x(3);
    x(0) = 1E0, x(1) = 0.0, x(2) = 0.0;
    double t = 0;
    double dt = 1E-6, h = 0;//initial step_size
    int steps = 0 , failed = 0;
    boost::numeric::odeint::adaptive_bdf2< boost::numeric::ublas::vector<double>, double > solver;
    
    while ( steps < 107)
   {
        myfile << t << "  " << x[0] << "  " << x[1] <<"  "<< x[2] <<"\n";
        cout <<" solution : " <<t << "  "<< x[0] <<"  " << x[1] << "  " << x[2]<< "\n" ;
        h = solver.do_step(make_pair( stiff_system() , stiff_system_jacobi() ), x , t , dt );
        //t = t + h;
        if ( h == 0 ) failed++;
        steps++;
    }
    myfile.close();
    cout << " steps : "<< steps << endl << "failed :" << failed <<  endl;
    
    return 0;
}
